...
const App: React.FC = () => {
  ...
  const setOrderTable = (v: number[] | ((prev: number[]) => number[])) => {
    const next = typeof v === "function" ? (v as any)(orderTableRef.current) : v;
  ...
  const setTransparentStartOrder = (v: number | ((prev: number) => number)) => {
    const next = typeof v === "function" ? (v as any)(transparentStartOrderRef.current) : v;
  ...
  const setNeedReconvert = (v: boolean | ((prev: boolean) => boolean)) => {
    const next = typeof v === "function" ? (v as any)(needReconvertRef.current) : v;
  ...
  const setLuaCodes = (v: string[][] | undefined | ((prev?: string[][]) => string[][] | undefined)) => {
  ...
  const setConvertOption = (v: ConvertOption | ((prev: ConvertOption) => ConvertOption)) => {
    const next = typeof v === "function" ? (v as any)(convertOptionRef.current) : v;
  ...
  const setLuaCodeOption = (v: LuaCodeOption | ((prev: LuaCodeOption) => LuaCodeOption)) => {
    const next = typeof v === "function" ? (v as any)(luaCodeOptionRef.current) : v;
  ...
  const handleWorkerMessage = useCallback((evt: MessageEvent<WorkerCommand>) => {
    const data = evt.data;
  ...
  const handleChangeConvertSettings = useCallback((opt: ConvertOption, need: boolean = false) => {
    setConvertOption((state) => {
      let ss: ConvertOption = { ...state };
