...
ctx.addEventListener("message", (evt: MessageEvent<WorkerCommand>) => {
  ...
  if (Worker) {
    if (!workerData.subWorker) {
      ...
      workerData.subWorker.addEventListener(
        "message",
  ...
  function commandFromSubWorker(data: WorkerCommand): boolean {
    if (ConvertSucceedCommand.is(data)) {
      ...
      if (!workerData.commandStartOffsetList[data.metaData.offsetListIndex]) {
        ...
        const vcmp = workerData.convRule.luaVCompress,
          hcmp = workerData.convRule.luaHCompress;
  ...
  if (!commandFromSubWorker(data)) {
    if (OpenFileCommand.is(data)) {
      ...
      for (let i = 0; i < cs.length; i += 4) {
        colorSet.push(
          new Color(cs[i], cs[i + 1], cs[i + 2], cs[i + 3], cs32[i / 4]),
        );
      }
      ...
      workerData.convData = workerData.rdata.map((v) =>
        data.colorPallete.indexOf(v),
      );
      ...
      for (
        let i = workerData.convRule.pictureOffsetX;
      ...
      for (
        let i = workerData.convRule.pictureOffsetY;
      ...
      workerData.commandStartOffsetList = new Array<boolean>(
        workerData.commandStartOffsetListX.length *
          workerData.commandStartOffsetListY.length,
      ).map(() => false);
...
function makeNextConvertData() {
  ...
  for (let x = 0; x < workerData.commandStartOffsetListX.length; x++) {
    for (let y = 0; y < workerData.commandStartOffsetListY.length; y++) {
      if (
        !workerData.commandStartOffsetList[
          y + workerData.commandStartOffsetListY.length * x
        ]
      ) {
  ...
  const d = new Uint32Array(
    workerData.convRule.luaCardWidth * workerData.convRule.luaCardHeight,
  );
